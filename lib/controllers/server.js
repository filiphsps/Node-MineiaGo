    ...
    command.registerCommand(
        ...
        (para, meta) => {
            chat.broadcast('private.' + meta.player.username,
                'This server is running ' + color('MineiaGo', 'purple') + ' version ' + color(pack.version, 'blue') + '. It\'s by ' + color(pack.author, 'purple'));
        });
    command.registerCommand(
        'mineiago:list',
        'list',
    ...
    Minecraft.ping({
        ...
        global.pc_ping = setInterval(() => {
            Minecraft.ping({
                ...
                log('Updated MOTD, maxPlayers & onlinePlayers', 0);
            });
        }, 30000);
